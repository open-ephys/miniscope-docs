
##################################
Open Ephys Miniscope Documentation
##################################

..  image:: /_static/images/miniscope-daq_ucla-miniscope-v4.webp
    :width:     100%
    :align:     center

Welcome to the documentation for the Miniscope System by Open Ephys! This site
is designed to help you get the most out of your Miniscope System, whether
you're a new user or an experienced developer.

The Miniscope V4 is a lightweight, open-source miniature microscope designed for
high-resolution fluorescent imaging in freely behaving animals. The Miniscope
ecosystem was originally designed by the Miniscope Team at UCLA. Here you can
find details about the versions we produce.

.. grid:: 3
    :class-container: gap-4

    .. grid-item-card:: Getting Started
        :link:          Getting-Started/index
        :link-type:     doc
        :link-alt:      link to getting started guide
        :img-top:       /_static/images/running.svg
        :img-alt:       Getting Started
        :shadow:        none
        :class-item:    hover-zoom
        :class-card:    card intro-card sd-border-0 text-center
        :class-title:   card-title sd-font-weight-normal
        :class-body:    card-body flex-fill
        :class-img-top: card-img-top card-img-top-sm

        Read this if you're just starting out.

    .. grid-item-card:: Hardware Guide
        :link:          Hardware-Guide/index
        :link-type:     doc
        :link-alt:      link to hardware guide
        :img-top:       /_static/images/noun_pcb.svg
        :img-alt:       Hardware Guide
        :shadow:        none
        :class-item:    hover-zoom
        :class-card:    card intro-card sd-border-0 text-center
        :class-title:   card-title sd-font-weight-normal
        :class-body:    card-body flex-fill
        :class-img-top: card-img-top card-img-top-sm

        Explore the details of the Miniscope hardware and how to build
        your own scopes.

    .. grid-item-card:: Software Guide
        :link:          Software-Guide/index
        :link-type:     doc
        :link-alt:      link to software guide
        :img-top:       /_static/images/noun_macbook.svg
        :img-alt:       Software Guide
        :shadow:        none
        :class-item:    hover-zoom
        :class-card:    card intro-card sd-border-0 text-center
        :class-title:   card-title sd-font-weight-normal
        :class-body:    card-body flex-fill
        :class-img-top: card-img-top card-img-top-sm

        Learn about software to run miniscope acquisition.



.. toctree::
    :hidden:
    :titlesonly:

    Getting-Started/index
    Hardware-Guide/index
    Software-Guide/index
    faq