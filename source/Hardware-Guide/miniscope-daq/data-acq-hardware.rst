...
..  important::  The Sync Output functionality has been corrected in firmware v1.1+ and software v0.4+ so that each toggle is precisely timed to the capture of individual frames and the FrameNumber counter properly increments by one each time a frame is captured. Use the latest :doc:`Miniscope DAQ firmware <firmware-update>` and the latest `OpenEphys.Miniscope Bonsai package <https://github.com/open-ephys/bonsai-miniscope/releases>`__ from Open Ephys to ensure hardware synchronization.
