...
- When a new firmware version is released, as a routine update. This should not
  happen often as we do not expect to release new firmware versions frequently,
...
required during normal operation and so are not installed by default. They must
be installed separately on each PC used for firmware updates.
